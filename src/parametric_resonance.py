...
class ParametricResonance(Scene):
    ...
    def generate_voltage_plot(self, axes):
        ...
        self.i = 0
        return axes.plot(lambda t: self.get_voltage_and_current(t)[0], [0, self.time.get_value(), self.dt]).set_color(YELLOW)

    # Generates the plot Current by Time
    def generate_current_plot(self, axes):
        self.q = self.c.get_value() * self.emf
        self.i = 0
        return axes.plot(lambda t: self.get_voltage_and_current(t)[1], [0, self.time.get_value(), self.dt]).set_color(BLUE)
